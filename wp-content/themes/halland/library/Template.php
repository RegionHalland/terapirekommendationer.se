...
class Template
{
    ...
    public function __construct()
    {
         ...
         * Set paths
         */
        $this->VIEWS_PATHS = apply_filters('Halland/blade/view_paths', array(
    ...
    public function adminFrontPageTemplates()
    {
        ...
        \Halland\Helper\Template::add(__('Page', 'halland'), \Halland\Helper\Template::locateTemplate('page.blade.php'));

        add_filter('gettext', function ($translation, $text, $domain) {
            if ($text == 'Default Template') {
                return __('Front page', 'halland');
            }
    ...
    {
        $directory = HALLAND_PATH . 'library/Controller/';

        foreach (@glob($directory . "*.php") as $file) {
            $class = '\Halland\Controller\\' . basename($file, '.php');
    ...
    {
        if ($view = \Halland\Helper\Template::locateTemplate('searchform.blade.php')) {
    ...
    public function load($template)
    {
        if ((is_page() || is_single() || is_front_page()) && !empty(get_page_template_slug()) && get_page_template_slug() != $template) {
            if (\Halland\Helper\Template::locateTemplate(get_page_template_slug())) {
                $template = get_page_template_slug();
            }
        }

        if (!\Halland\Helper\Template::isBlade($template)) {
            $path = $template;
            ...
            if (file_exists($path)) {
                ...
                if (current_user_can('administrator')) {
                    \Halland\Helper\Notice::add('<strong>' . __('Admin notice', 'halland') . ':</strong> ' . sprintf(__('View [%s] was not found. Defaulting to [page.blade.php].', 'halland'), $template), 'warning', 'pricon pricon-notice-warning');
                }

                $template = \Halland\Helper\Template::locateTemplate('views/page.blade.php');
            }
        }
    ...
    public function loadController($template)
    {
        ...
        do_action('Halland/blade/before_load_controller');
        ...
        $controller = \Halland\Helper\Controller::locateController($template);

        if (!$controller) {
            $controller = \Halland\Helper\Controller::locateController('BaseController');
        }

        $controller = apply_filters('Halland/blade/controller', $controller);

        require_once $controller;
        $namespace = \Halland\Helper\Controller::getNamespace($controller);
        $class = '\\' . $namespace . '\\' . basename($controller, '.php');

        do_action('Halland/blade/after_load_controller');
    ...
    {
        $data = apply_filters('Halland/blade/data', $data);
    ...
    public function addTemplateFilters()
    {
        ...
        $types = apply_filters('Halland/blade/template_types', $types);
        ...
        if (isset($types) && !empty($types) && is_array($types)) {
            foreach ($types as $key => $type) {
                add_filter($key . '_template', function ($original) use ($key, $type, $types) {
                    ...
                    $templatePath = \Halland\Helper\Template::locateTemplate($type);
                    ...
                    if (is_post_type_archive() && isset($wp_query->query['post_type'])) {
                        ...
                        $found = \Halland\Helper\Template::locateTemplate($search);
