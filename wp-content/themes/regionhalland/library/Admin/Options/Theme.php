<?php

namespace RegionHalland\Admin\Options;

class Theme
{
    public function __construct()
    {
        if (function_exists('acf_add_options_page')) {
            $themeOptionsCapability = 'administrator';
            $themeOptionsParent = 'themes.php';

            add_action('admin_menu', function () use ($themeOptionsParent) {
                global $submenu;
                $submenu[$themeOptionsParent][] = array( '', 'read', '', '', 'wp-menu-separator');
            }, 11);

            acf_add_options_sub_page(array(
                'page_title'    => __('Theme Options', 'regionhalland'),
                'menu_title'    => __('Theme Options', 'regionhalland'),
                'parent_slug'   => $themeOptionsParent,
                'capability'    => $themeOptionsCapability,
                'redirect'      => false,
                'icon_url'      => 'data:image/svg+xml;base64,'
            ));

            acf_add_options_sub_page(array(
                'page_title'    => __('Navigation', 'regionhalland'),
                'menu_title'    => __('Navigation', 'regionhalland'),
                'parent_slug'   => $themeOptionsParent,
                'capability'    => $themeOptionsCapability,
                'menu_slug'     => 'acf-options-navigation'
            ));

            acf_add_options_sub_page(array(
                'page_title'    => __('Header', 'regionhalland'),
                'menu_title'    => __('Header', 'regionhalland'),
                'parent_slug'   => $themeOptionsParent,
                'capability'    => $themeOptionsCapability,
                'menu_slug'     => 'acf-options-header'
            ));

            acf_add_options_sub_page(array(
                'page_title'    => __('Content', 'regionhalland'),
                'menu_title'    => __('Content', 'regionhalland'),
                'parent_slug'   => $themeOptionsParent,
                'capability'    => $themeOptionsCapability,
                'menu_slug'     => 'acf-options-content'
            ));

            acf_add_options_sub_page(array(
                'page_title'    => __('Footer', 'regionhalland'),
                'menu_title'    => __('Footer', 'regionhalland'),
                'parent_slug'   => $themeOptionsParent,
                'capability'    => $themeOptionsCapability,
                'menu_slug'     => 'acf-options-footer'
            ));

            acf_add_options_sub_page(array(
                'page_title'    => __('Search', 'regionhalland'),
                'menu_title'    => __('Search', 'regionhalland'),
                'parent_slug'   => $themeOptionsParent,
                'capability'    => $themeOptionsCapability,
                'menu_slug'     => 'acf-options-search'
            ));

            acf_add_options_sub_page(array(
                'page_title'    => __('Blog & Archives', 'regionhalland'),
                'menu_title'    => __('Blog & Archives', 'regionhalland'),
                'parent_slug'   => $themeOptionsParent,
                'capability'    => $themeOptionsCapability,
                'menu_slug'     => 'acf-options-archives'
            ));

            acf_add_options_sub_page(array(
                'page_title'    => '404',
                'menu_title'    => '404',
                'parent_slug'   => $themeOptionsParent,
                'capability'    => $themeOptionsCapability,
                'menu_slug'     => 'acf-options-404'
            ));

            acf_add_options_sub_page(array(
                'page_title'    => 'Google Translate',
                'menu_title'    => 'Google Translate',
                'parent_slug'   => $themeOptionsParent,
                'capability'    => $themeOptionsCapability,
                'menu_slug'     => 'acf-options-google-translate'
            ));

            acf_add_options_sub_page(array(
                'page_title'    => __('Content editor', 'regionhalland'),
                'menu_title'    => __('Content editor', 'regionhalland'),
                'parent_slug'   => $themeOptionsParent,
                'capability'    => $themeOptionsCapability,
                'menu_slug'     => 'acf-options-content-editor'
            ));

            acf_add_options_sub_page(array(
                'page_title'    => __('Custom Post Types', 'regionhalland'),
                'menu_title'    => __('Post Types', 'regionhalland'),
                'parent_slug'   => $themeOptionsParent,
                'capability'    => $themeOptionsCapability,
                'menu_slug'     => 'acf-options-post-types'
            ));

            acf_add_options_sub_page(array(
                'page_title'    => __('Custom Taxonomies', 'regionhalland'),
                'menu_title'    => __('Taxonomies', 'regionhalland'),
                'parent_slug'   => $themeOptionsParent,
                'capability'    => $themeOptionsCapability,
                'menu_slug'     => 'acf-options-taxonomies'
            ));

            acf_add_options_sub_page(array(
                'page_title'    => __('Custom CSS/JS Editor', 'regionhalland'),
                'menu_title'    => __('Custom CSS/JS', 'regionhalland'),
                'parent_slug'   => $themeOptionsParent,
                'capability'    => $themeOptionsCapability,
                'menu_slug'     => 'acf-options-css'
            ));
        }
    }
}
