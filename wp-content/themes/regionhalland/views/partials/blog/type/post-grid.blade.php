...
$thumbnail = regionhalland_get_thumbnail_source(
    $post->ID,
    array(500, 500)
);

$columnSize = \RegionHalland\Controller\Archive::getColumnSize();
?>
<div class="{{ $columnSize }}">
    <a href="{{ the_permalink() }}" class="box box-post-brick <?php echo $grid_alter ? 'brick-columns-' . $gridSize . '"' : ''; ?>">
        ...
        <div class="box-image" {!! $thumbnail ? 'style="background-image:url(' . $thumbnail . ');"' : '' !!}>
            <img src="{{ regionhalland_get_thumbnail_source(null,array(500,500)) }}" alt="{{ the_title() }}">
        </div>
        @endif
