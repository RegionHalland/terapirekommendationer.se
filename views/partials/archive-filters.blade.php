    ...
    <form method="get" action="{{ $archiveUrl }}" class="container" id="archive-filter">
        <div class="grid">
            ...
            <div class="grid-sm-12 grid-md-auto">
                <label for="filter-keyword" class="text-sm sr-only"><strong><?php _e('Search', 'regionhalland'); ?>:</strong></label>
                <div class="input-group">
                    <span class="input-group-addon"><i class="fa fa-search"></i></span>
                    <input type="text" name="s" id="filter-keyword" class="form-control" value="{{ !empty(get_search_query()) ? get_search_query() : '' }}" placeholder="<?php _e('Search', 'regionhalland'); ?>">
                </div>
            </div>
            @endif
            ...
            <div class="grid-sm-12 grid-md-auto">
                <label for="filter-date-from" class="text-sm sr-only"><strong><?php _e('Date published', 'regionhalland'); ?>:</strong></label>
                <div class="input-group">
                    <span class="input-group-addon"><?php _e('From', 'regionhalland'); ?>:</span>
                    <input type="text" name="from" placeholder="<?php _e('From date', 'regionhalland'); ?>…" id="filter-date-from" class="form-control datepicker-range datepicker-range-from" value="{{ isset($_GET['from']) && !empty($_GET['from']) ? sanitize_text_field($_GET['from']) : '' }}" readonly>
                    <span class="input-group-addon"><?php _e('To', 'regionhalland'); ?>:</span>
                    <input type="text" name="to" placeholder="<?php _e('To date', 'regionhalland'); ?>" class="form-control datepicker-range datepicker-range-to" value="{{ isset($_GET['to']) && !empty($_GET['to']) ? sanitize_text_field($_GET['to']) : '' }}" readonly>
                </div>
            </div>
            @endif
            ...
            @if($queryString)
                <div class="grid-sm-12 hidden-sm hidden-xs grid-md-fit-content">
                    <a class="btn btn-block pricon pricon-close pricon-space-right" href="{{ $archiveUrl }}"><?php _e('Clear filters', 'regionhalland'); ?></a>
                </div>
            @endif
            <div class="grid-sm-12 grid-md-fit-content">
                <input type="submit" value="<?php _e('Search', 'regionhalland'); ?>" class="btn btn-primary btn-block">
            </div>
        </div>
                ...
                <div class="grid-md-4">
                    <div class="box box-panel box-panel-secondary" data-equal-item>
                        ...
                        <div class="box-content">
                            <?php $taxonomy->slug = $taxKey; $dropdown = \RegionHalland\Content\PostFilters::getMultiTaxDropdown($taxonomy, 0, 'list-hierarchical'); ?>
                            {!! $dropdown !!}
                        </div>
                    </div>
                </div>
