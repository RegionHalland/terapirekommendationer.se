    ...
    <div class="grid">
        ...
        <div class="{{ $cols }}">
            ...
            <div class="grid">
                                    ...
                                    <div class="grid">
                                        ...
                                        <h2><a href="{{ esc_url(add_query_arg('date', preg_replace('/\D/', '', $post->start_date), the_permalink())) }}">{{ the_title() }}</a></h2>
                                            <div class="event-archive_meta">
                                                ...
                                                <strong><?php _ex('Date', 'Event archive', 'regionhalland'); ?>:</strong>
                                                   {{ \RegionHalland\Helper\Event::formatEventDate($post->start_date, $post->end_date) }}
                                                </small></p>

                                                <?php $location = get_field('location'); ?>
                                                @if (!empty($location['title']))
                                                    <p><small><i class="pricon pricon-location-pin"></i>
                                                    <strong><?php _ex('Location', 'Event archive','regionhalland'); ?>:</strong> {{ $location['title'] }}
                                                    </small></p>
                                                @endif
                                            </div>
                                        ...
                                        @if (regionhalland_get_thumbnail_source(null,array(400,250)))
                                            <div class="grid-md-3">
                                                <img src="{{ regionhalland_get_thumbnail_source(null,array(400,250)) }}">
                                            </div>
                                        @endif
                                    </div>
                                </li>
                ...
                @else
                    <div class="grid-xs-12">
                        <div class="notice info pricon pricon-info-o pricon-space-right"><?php _e('No posts to show', 'regionhalland'); ?>…</div>
                    </div>
                @endif
            </div>
